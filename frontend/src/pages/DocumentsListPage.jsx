...
import { deleteDocument, getHelperDocuments, getHelpers } from "../api.js";
import AddActionButton from "../components/AddActionButton.jsx";

export default function DocumentsListPage({ token }) {
  ...
  const load = async (selectedHelperId = helperId) => {
    try {
      ...
      if (activeHelperId) {
        const documentData = await getHelperDocuments(token, activeHelperId);
        setRecords(documentData);
      } else {
        setRecords([]);
      }
    } catch (error) {
  ...
  return (
    <section className="panel">
      <div className="section-header">
        <h2>Helper Documents</h2>
        <AddActionButton label="Add Document" onClick={() => navigate("/admin/documents/new")} />
      </div>
        ...
        <tbody>
          {records.map((record) => (
            <tr key={record.id}>
              ...
              <td>{record.document_status}</td>
              <td>{record.expiry_date || "-"}</td>
              <td className="row-actions">
                <button type="button" className="secondary" onClick={() => navigate(`/admin/documents/${record.id}/edit`)}>
                  Edit
                </button>
